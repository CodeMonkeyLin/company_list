...
class CompanyListSpider(CrawlSpider):
    ...
    def parse_list(self, response):
        ...
        company_codes = sel.xpath("//tr[@class='even']/td[2]/text()").extract()
        ...
        try:
            for index in range(len(detail_hrefs)):
                ...
                print cur.execute
                count = cur.execute('SELECT company_code FROM company_list WHERE company_code = "%s"' % company_code)
                if cur.rowcount == 0:
                    print 1
                    cur.execute(
                        'INSERT INTO company_list (`company_code`,`detail_href`) values ("%s","%s")' % (company_code,detail_href))
                    conn.commit()
